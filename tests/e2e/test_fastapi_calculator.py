...
def test_create_calculation_modulus_by_zero_rejected(base_url: str):
    user_data = {
        "first_name": "Calc",
        "last_name": "ModZero",
    ...
    response = requests.post(url, json=payload, headers=headers)
    assert response.status_code == 422, f"Expected 422 for modulus by zero, got {response.status_code}. Response: {response.text}"
